...
from src.utils import (extract_tables_from_markdown,
                       save_data_to_json_file, set_env, saving_list_of_dfs)
...
if __name__ == '__main__':
    ...
    for paper_path in (pbar:=tqdm(papers_dir_list)):
        ...
        if paper_path.suffix == '.pdf':
            ...
            markdown_file_path = parse_pdf_with_marker(str(paper_path), marker_output_dir)
            ...
            try:
                ...
                manual_tables_path = os.path.join(paper_output_dir_path, "manual_extracted_tables")
                os.makedirs(manual_tables_path, exist_ok=True)
                saving_list_of_dfs(extracted_tables_with_markdown, Path(manual_tables_path))

            except Exception as e:
                logger.error(f"Extracting with markdown yielded this error:{e}")

            # Saving tables from tabula
            tabula_tables_file_path = os.path.join(paper_output_dir_path, "tabula_extracted_files")
            ...
            llm_tables_file_path = os.path.join(paper_output_dir_path, "llm_extracted_tables")
            os.makedirs(llm_tables_file_path, exist_ok=True)
            parse_markdown_sections(pdf_sections_content, llm_tables_file_path, model_name="phi3:medium")
        else:
            logger.warning(f"The provided path does not have required extension: {paper_path.suffix} instead of PDF")
            continue
