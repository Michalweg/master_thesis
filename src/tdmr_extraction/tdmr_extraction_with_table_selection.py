...
PROBLEMATIC_PAPERS = [
    ...
    "1905.12598",
    "1804.09849",
]  # Those for which regular approach yielded 0 %
...
def pick_optimal_source_table_for_given_triplet(
    ...
    if structured_output:
        response = get_openai_model_structured_response(
            prompt, result_object, system_prompt=system_prompt
        )
        if response:
            response = response.model_dump()
    else:
        response = get_openai_model_response(
            prompt, result_object, system_prompt=system_prompt
        )
...
def formate_prepared_dicts_into_str(prepared_dicts: list[PreparedTablesDict]) -> str:
    ...
    for prepared_dict in prepared_dicts:
        dicts_into_str += PREPARED_DICT_INTO_STR_TEMPLATE.format(
            table_markdown=prepared_dict.table,
...
if __name__ == "__main__":
    ...
    extracted_triplet_dir_path = "triplets_normalization/triplets_normalization_from_entire_document_refined_prompt_gpt-4-turbo_gemini"
    tdmr_extraction_dir = (
        f"tdmr_extraction/{MODEL_NAME}/with_picking_optimal_table_and_gemini_prompt"
    )
    ...
    # papers_with_extracted_tables_just_names = PROBLEMATIC_PAPERS
