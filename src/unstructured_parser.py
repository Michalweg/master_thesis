...
def assign_table_text_and_caption_to_a_table(table_text_captions_dict: dict, papers_tables_dir: str):
    ...
    for paper_table_path in papers_tables_dir:
        ...
        for index in range(len(tables_captions)):
            table_text, table_caption = papers_tables_from_dict[index], tables_captions[index]
            prompt = PromptTemplate(template=PROMPT_FOR_ASSIGNING_TEXT_AND_CAPTION).format(
                text_table=table_text,
                table_caption=table_caption,
                csv_table=pd.read_html(tables_htmls[index]),
            )
            model_output = send_request_to_the_model_with_ollama(prompt, model_name=MODEL_NAME_FOR_CAPTION_ASSIGNMENT,
                                                                 url='')
...
if __name__ == "__main__":
    ...
    for paper_path in (pbar := tqdm(papers_dir_list)):
        ...
        if paper_path.suffix == ".pdf":
            elements = partition_pdf(paper_path, strategy='hi_res', infer_table_structure=True)
            for i, el in enumerate(elements):
                if el.category == "Table":
                    try:
                        if (elements[i - 1].text not in captions) and (
                                (elements[i - 1].category == 'FigureCaption') or (elements[i - 1].text[:5] == 'Table')):
            ...
            save_dict_to_json(tables_and_captions_dict, os.path.join(tables_text_with_captions_dir, f"{paper_path.stem}_tables_with_captions.json"))
