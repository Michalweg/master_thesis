...
from src.parser_prompts import triplets_extraction_prompt_gpt_4
...
def remove_table_content_from_parsed_md_file():
    papers_without_tables_dir = f"parsing_experiments/30_11_2024_without_extracted_table_content_in_section"
    create_dir_if_not_exists(Path(papers_without_tables_dir))

    parsed_papers_dir_name = "parsing_experiments/11_11_2024_fixed_section_extraction_llama3.1"
    ...
    for paper_path in (pbar := tqdm(papers_dir_list)):
        ...
        marker_output_md_file = f"{str(paper_path)}/marker_output/{paper_name}/{paper_name}.md"
        marker_output_metadata_file_path = f"{str(paper_path)}/marker_output/{paper_name}/{paper_name}_meta.json"
        paper_without_table_path_dir = os.path.join(papers_without_tables_dir, paper_name)
        create_dir_if_not_exists(Path(paper_without_table_path_dir))
        paper_without_table_path = os.path.join(paper_without_table_path_dir, paper_name + ".md")
        remove_table_data_from_markdown(marker_output_md_file, paper_without_table_path)

        extracted_sections = extract_pdf_sections_content(paper_without_table_path, marker_output_metadata_file_path)
...
if __name__ == "__main__":
    ...
    triplets_output_dir_without_table_content = f"triplets_extraction/from_each_section_with_table_{MODEL_NAME}"
    ...
    for paper_path in parsed_papers_without_table_content:
            paper_name_output_path = Path(f"{triplets_output_dir_without_table_content}/{paper_path.name}")
