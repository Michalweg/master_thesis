...
from prompts.parser_prompts import (gpt_4_prompt, gpt_4o_prompt_rana,
                                    llama_31_7B_prompt)
...
def send_request_to_the_model_with_ollama(
    prompt: str, model_name: str, url: str = "http://localhost:11434/api/generate"
) -> str:

    import subprocess

    data = json.dumps(
        {"model": model_name, "prompt": prompt, "stream": False, "temperature": 0.0}
    )
...
def parse_markdown_with_llm(
    ...
    if len(file_content) > 10:
        ...
        if model_response:
            parsed_response_file_path = parse_model_response(
                model_response,
                output_parser,
                output_dir,
                file_content,
            ...
            if parsed_response_file_path:
                ...
                if dfs:
                    for i, df in enumerate(dfs):
                        df.to_csv(
                            os.path.join(
                                os.path.dirname(parsed_response_file_path),
                                Path(parsed_response_file_path).stem + f"{str(i)}.csv",
                            ),
                            index=False,
                        )
                else:
                    ...
                    save_str_as_markdown(
                        os.path.join(base_path, file_name.split(".")[0] + "md"),
                        "No table could be extracted from this: \n" + model_response,
                    )
        else:
    ...
    try:
        ...
        if parsed:
            parsed_response_file_path = os.path.join(
                str(output_dir),
                Path(markdown_file_path).stem + f"len_{len(file_content)}" + ".json",
            )
            save_dict_to_json(parsed, parsed_response_file_path)
        else:
    ...
    if all_dfs_in_json_file:
        for table_data in all_dfs_in_json_file:
            ...
            if data:
                ...
                logger.info(
                    f"Extracted section: {str(json_file_path).split('_')[0]} does not contain any table"
                )
    return dfs
